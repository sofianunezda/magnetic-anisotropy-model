...
carpeta_resultados = carpeta_actual / "results"
...
# 7. GUARDADO DE RESULTADOS EN ARCHIVO TXT
ruta_resultados = carpeta_resultados / "results.txt"

with open(ruta_resultados, "w", encoding="utf-8") as archivo:
    archivo.write("="*45 + "\n")
    archivo.write("MODELO DE ANISOTROPÍA MAGNÉTICA\n")
    archivo.write("="*45 + "\n")

    archivo.write("\n" + "="*45)
    archivo.write("\nPARÁMETROS FÍSICOS\n")
    archivo.write(f"K_u = {K_u:.2e} J/m\u00B3\n")
    archivo.write(f"M_s = {M_s:.2e} A/m\n")
    archivo.write(f"H   = {H:.2e} A/m\n")
    archivo.write(f"H_k = {H_k:.2e} A/m\n")

    archivo.write("\nCASO BASE\n")
    archivo.write(f"\u03C6_H   = {phi_H:.1f}\u00B0\n")
    archivo.write(f"\u03B8_eq  = {theta_eq:.2f}\u00B0\n")
    archivo.write(f"E_min = {E_min:.2f} J/m\u00B3\n")

    archivo.write("\nCAMPO PARALELO AL EJE FÁCIL\n")
    archivo.write(f"\u03C6_H   = {phi_H_paralelo:.1f}\u00B0\n")
    archivo.write(f"\u03B8_eq  = {theta_eq_paralelo:.2f}\u00B0\n")
    archivo.write(f"E_min = {E_min_paralelo:.2f} J/m\u00B3\n")

    archivo.write("\nCAMPO PERPENDICULAR AL EJE FÁCIL\n")
    archivo.write(f"\u03C6_H   = {phi_H_perpendicular:.1f}\u00B0\n")
    archivo.write(f"\u03B8_eq  = {theta_eq_perpendicular:.2f}\u00B0\n")
    archivo.write(f"E_min = {E_min_perpendicular:.2f} J/m\u00B3\n")

    archivo.write("\nCOMPARACIÓN SEGÚN LA INTENSIDAD DEL CAMPO\n")
    archivo.write(f"H = {H_bajo:.2e} A/m -> " f"\u03B8_eq  = {theta_eq_bajo:.2f}\u00B0\n")
    archivo.write(f"H = {H_medio:.2e} A/m -> " f"\u03B8_eq  = {theta_eq_medio:.2f}\u00B0\n")
    archivo.write(f"H = {H_alto:.2e} A/m -> " f"\u03B8_eq  = {theta_eq_alto:.2f}\u00B0\n")

    archivo.write("\nCOMPARACIÓN SEGÚN LA DIRECCIÓN DEL CAMPO\n")
    archivo.write(f"\u03C6_H = {phi_H_0:.0f}\u00B0 -> " f"\u03B8_eq  = {theta_eq_phi_0:.2f}\u00B0\n")
    archivo.write(f"\u03C6_H = {phi_H_45:.0f}\u00B0 -> " f"\u03B8_eq  = {theta_eq_phi_45:.2f}\u00B0\n")
    archivo.write(f"\u03C6_H = {phi_H_90:.0f}\u00B0 -> " f"\u03B8_eq  = {theta_eq_phi_90:.2f}\u00B0\n")
    archivo.write("="*45 + "\n")
    archivo.write("\nResultados generados automáticamente por magnetic_anisotropy_model.py\n")
